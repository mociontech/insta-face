...
export async function uploadUserPhotoToFirebase(base64Image) {
  try {
    const id = Date.now();
    const storageRef = ref(storage, `f1Hp/userPhotos/${id}.jpg`);
    await uploadString(storageRef, base64Image, "data_url");
    const url = `https://storage.googleapis.com/f1-sap.appspot.com/f1Hp/userPhotos/${id}.jpg`;
...
export async function uploadGeneratedPhotoToFirebase(blob) {
  try {
    const id = Date.now();
    const storageRef = ref(storage, `f1Hp/generatedPhotos/${id}.jpeg`);
    await uploadBytes(storageRef, blob);
    await getDownloadURL(storageRef);
    const url = `https://storage.googleapis.com/f1-sap.appspot.com/f1Hp/generatedPhotos/${id}.jpeg`;
